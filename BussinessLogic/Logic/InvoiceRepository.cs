            ...
            try
            {
                ...
                using (HttpClient httpClient = new HttpClient(handler))
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            throw new ApiException((int)response.StatusCode, "No se encontró la factura con el DocNum especificado.");
                        }
                    }
                    else
                    {
                        var errorResponse = await response.Content.ReadAsStringAsync();
                        throw new ApiException((int)response.StatusCode, $"Error al obtener la factura: {errorResponse}");
                    }
                }
            }
            catch (Exception ex)
